"""PPOB BUMDes starter. Secrets: SUPABASE_URL and SUPABASE_KEY."""
...
if page == "Dashboard":
    ...
    if file:
        raw=pd.read_excel(file); st.dataframe(raw.head(),use_container_width=True)
        cols=list(raw.columns)
        t=st.selectbox("Kolom tanggal",cols); j=st.selectbox("Kolom jenis",cols); n=st.selectbox("Kolom nama",cols); total=st.selectbox("Kolom total bayar",cols); modal=st.selectbox("Kolom modal",cols); default=st.selectbox("Status",STATUSES)
        if st.button("Validasi & import"):
            valid=[]; errors=[]
            for i,x in raw.iterrows():
                dt=pd.to_datetime(x[t],errors="coerce"); a=pd.to_numeric(x[total],errors="coerce"); b=pd.to_numeric(x[modal],errors="coerce")
                if pd.isna(dt) or pd.isna(a) or pd.isna(b) or a<0 or b<0 or a<b: errors.append({"baris":i+2,"error":"Tanggal/angka tidak valid atau total < modal"})
                else: valid.append({"tanggal":dt.date().isoformat(),"jenis":str(x[j]),"nama":None if pd.isna(x[n]) else str(x[n]),"total_bayar":float(a),"modal":float(b),"status":default})
            if errors: st.dataframe(pd.DataFrame(errors),hide_index=True)
            if valid:
                try: supabase.table("transaksi").insert(valid).execute(); supabase.table("riwayat_import").insert({"nama_file":file.name,"jumlah_data":len(valid)+len(errors),"berhasil":len(valid),"gagal":len(errors)}).execute(); log(f"Import {file.name}","transaksi"); st.success(f"{len(valid)} data diimpor.")
                except Exception as e: st.error(f"Import gagal: {e}")
